  ...
  return new Promise(async function(resolve, reject) {
    try {
      ...
      if (rsiData.slice(-1)[0].rsi > bottom && rsiData.slice(-1)[0].rsi < top) {
        ...
        if (oTrend.recentGains > oTrend.recentLosses && oTrend.countGains > oTrend.last * 0.65) {
          objRSI[0].oper = "buy";
          objRSI[0].factor = 1;
          objRSI[0].rules += " with uptrend";
        }
        if (oTrend.recentGains < oTrend.recentLosses && oTrend.countGains < oTrend.last * 0.35) {
          objRSI[0].oper = "sell";
          objRSI[0].factor = 1;
